...
class FlexARInferenceSolver:
    ...
    def __init__(self, model_path, precision, target_size=512, tokenizer_path=None):
        ...
        self.item_processor = FlexARItemProcessor(with_decoder=True, target_size=target_size, tokenizer_path=tokenizer_path)
    ...
    def generate(
        ...
        try:
            ...
            with torch.cuda.amp.autocast(dtype=self.dtype):
                ...
                logger.info(f"Raw generation result: {generation_result}")
                
                if len(generation_result) > 0 and generation_result[-1] == 8710:
                    generation_result = generation_result[:-1]
                
                logger.info(f"Processed generation result: {generation_result}")

            generated_text, generated_images = self.decode_ids(generation_result)
            
            if generated_images:
                logger.info(f"Generated image count: {len(generated_images)}")
                logger.info(f"First generated image size: {generated_images[0].size}, mode: {generated_images[0].mode}")
    ...
    def decode_image(self, tokens: List[int]):
        logger.info(f"Decoding image from tokens: {tokens[:10]}...")  # Log first 10 tokens
        try:
