...
class FlexARItemProcessor(MMConvItemProcessor):
    ...
    def __init__(
        ...
        # Use the provided tokenizer_path if available
        if tokenizer_path:
            text_tokenizer_path = os.path.join(tokenizer_path, "text_tokenizer.json")
            vqgan_yaml_path = os.path.join(tokenizer_path, "vqgan.yaml")
            vqgan_ckpt_path = os.path.join(tokenizer_path, "vqgan.ckpt")
        else:
            text_tokenizer_path = "./ckpts/chameleon/tokenizer/text_tokenizer.json"
            vqgan_yaml_path = "./ckpts/chameleon/tokenizer/vqgan.yaml"
            vqgan_ckpt_path = "./ckpts/chameleon/tokenizer/vqgan.ckpt"

        tokenizer_data = load_json_file(text_tokenizer_path)
        self.chameleon_ori_vocab = chameleon_vae_ori.VocabInfo(tokenizer_data["model"]["vocab"])
    ...
    def decode_image(self, tokens: List[int]) -> Image.Image:
        logger.info(f"Decoding image from tokens: {tokens[:10]}...")
        try:
            ...
            image = self.chameleon_ori_image_tokenizer.pil_from_img_toks(tokens, h_latent_dim, w_latent_dim)
