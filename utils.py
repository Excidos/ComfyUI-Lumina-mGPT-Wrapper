...
# Update this path to the correct location of the tokenizer
DEFAULT_TOKENIZER_PATH = os.path.join(os.path.dirname(__file__), "Lumina-mGPT", "lumina_mgpt", "ckpts", "chameleon", "tokenizer")
...
def download_and_load_model(model_name, precision, target_size):
    ...
    for file in tokenizer_files:
        file_path = os.path.join(DEFAULT_TOKENIZER_PATH, file)
        if not os.path.exists(file_path):
            logger.error(f"Required tokenizer file {file} not found in {DEFAULT_TOKENIZER_PATH}")
            raise FileNotFoundError(f"Required tokenizer file {file} not found")
...
class CustomFlexARInferenceSolver:
    ...
    def create_logits_processor(self, cfg, image_top_k):
        return CustomLogitsProcessor(cfg, image_top_k, text_top_k=50, vocab_size=self.model.config.vocab_size)
